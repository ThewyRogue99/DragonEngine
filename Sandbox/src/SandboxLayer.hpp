...
public:
	...
	{
		SquareVertexArray = Engine::VertexArray::Create();
		...
		SquareIndexBuffer = Engine::IndexBuffer::Create(indices, 6);
		...
		GridVertexArray = Engine::VertexArray::Create();
		...
		GridVertexBuffer = Engine::VertexBuffer::Create(GridVertices, sizeof(GridVertices));
		...
		GridIndexBuffer = Engine::IndexBuffer::Create(indices, 6);
	...
	void OnUpdate(Engine::Timestep DeltaTime) override
	{
		...
		for (int y = -(GridSize / 2); y < (GridSize / 2); y++)
		{
			for (int x = -(GridSize / 2); x < (GridSize / 2); x++)
			{
				...
				Engine::Renderer::Submit(GridShader, GridVertexArray, transform);
			}
		}

		SquareTexture->Bind();
		Engine::Renderer::Submit(SquareShader, SquareVertexArray);
